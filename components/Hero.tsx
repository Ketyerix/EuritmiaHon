...
const Hero: React.FC = () => {
  ...
  return (
    <section id="hero" className="relative pt-32 pb-16 overflow-hidden min-h-screen flex flex-col items-center justify-center">
      ...
      <div 
        className="relative w-full max-w-4xl h-[250px] md:h-[380px] flex items-center justify-center mb-8"
      ...
      >
        {CAROUSEL_ITEMS.map((item, index) => {
          ...
          return (
            <div
              key={index}
              className="absolute w-[240px] md:w-[480px] h-[170px] md:h-[320px] transition-all duration-700 ease-in-out shadow-2xl overflow-hidden cursor-pointer rounded-[1.5rem] group"
              ...
              {item.type === 'video' && (
                <div className={`absolute inset-0 flex items-center justify-center transition-opacity duration-500 ${isActive ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
                   <a 
                     ...
                     className="w-16 h-16 bg-earth rounded-full flex items-center justify-center border border-white/20 hover:scale-110 hover:bg-earthDark transition-all duration-300 shadow-lg"
